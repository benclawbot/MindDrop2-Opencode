...
export const NewTaskInput = forwardRef<NewTaskInputHandle, NewTaskInputProps>(({ onAddTask, onUpdateTask, allTags, onAddTag, onCancel }, ref) => {
  ...
  const processTags = (text: string): string[] => {
    ...
    hashtags.forEach(ht => {
      ...
      let existingTag = allTags.find(t => t.name.toLowerCase() === tagName);
      if (!existingTag) {
  ...
  const executeTaskCreation = async (taskInput: string) => {
    ...
    try {
      const analysis = await AIService.analyzeNewTask(cleanedInput, tempTask.description, imageBase64 || undefined);
      const aiTagIds = analysis.tags ? processTags(analysis.tags.map((t: string) => `#${t}`).join(' ')) : [];
      ...
      const totalRemaining = subtasks.length > 0 
  ...
  return (
    <div className={`relative transition-all duration-300 z-50 max-w-lg mx-auto ${isExpanded ? 'bg-white dark:bg-stone-900 rounded-[2rem] shadow-2xl p-6 border border-stone-200 dark:border-white/5' : 'bg-white/90 dark:bg-stone-800/90 rounded-2xl shadow-md border border-stone-200 dark:border-white/5'}`}>
      <form onSubmit={handleSubmit} className="flex flex-col">
        <div className="flex items-center h-12">
            ...
            {!isExpanded && (
              <button type="submit" disabled={!title.trim() && !imageBase64} className="mr-3 ml-2 p-2 bg-indigo-600 text-white rounded-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-30">
           ...
           <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              {imageBase64 && (
                <div className="relative w-full aspect-video rounded-2xl overflow-hidden border border-stone-200 dark:border-white/5 bg-stone-50 dark:bg-black/20 group">
                  <img src={imageBase64} className="w-full h-full object-cover" alt="Vision input" />
                  <button 
              ...
              <div className="flex justify-between items-center px-1">
                <span className="text-[10px] font-black uppercase text-stone-300 tracking-[0.2em]">Press Enter to drop task</span>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => { setIsExpanded(false); onCancel?.(); }} className="text-[11px] font-black uppercase text-stone-400 px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 rounded-xl transition-colors">Cancel</button>
                  <button type="submit" disabled={(!title.trim() && !imageBase64) || isAnalyzing} className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50">
